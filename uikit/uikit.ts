...
// スピンボタン
//
const spinInput = document.getElementsByClassName('spin-input') as HTMLCollectionOf<HTMLElement>;
for(let i=0; i<spinInput.length; i++){
  const spinInputId = spinInput[i].getAttribute('id');
  const spinInputIdEle:any = <HTMLInputElement>document.getElementById(spinInputId);

  const spinPlus = document.getElementsByClassName('spin-plus') as HTMLCollectionOf<HTMLFormElement>;
  ...
  const spinMinus = document.getElementsByClassName('spin-minus') as HTMLCollectionOf<HTMLFormElement>;
